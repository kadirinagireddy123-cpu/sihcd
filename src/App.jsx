...
export default function App() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setTelemetry(prev => ({
        rainfall: parseFloat((prev.rainfall + (Math.random() * 0.4 - 0.1)).toFixed(1)),
        soilMoisture: Math.max(85, Math.min(99, prev.soilMoisture + (Math.random() > 0.5 ? 1 : -1))),
        rssi: Math.max(-90, Math.min(-65, prev.rssi + (Math.random() > 0.5 ? 1 : -1))),
        meshPing: Math.max(8, Math.min(22, prev.meshPing + (Math.random() > 0.5 ? 1 : -1))),
      }));
    }, 3000);
