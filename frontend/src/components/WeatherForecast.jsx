...
export default function WeatherForecast() {
  const [districtData, setDistrictData] = useState({ districts: DISTRICTS, hourly: HOURLY_FORECAST, sevenDay: SEVEN_DAY });
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.districts && data.districts.length > 0) {
            ...
            setSelectedDistrict(prev => data.districts.find(d => d.id === prev.id) || data.districts[0]);
          }
        }
      } catch (err) {
        console.warn('Weather API offline, using cached fallback data', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const activeDistricts = districtData.districts || DISTRICTS;
  const activeHourly    = districtData.hourly || HOURLY_FORECAST;
  const activeSevenDay  = districtData.sevenDay || SEVEN_DAY;
  const alertCfg = ALERT_CONFIG[selectedDistrict?.alert || 'GREEN'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {activeDistricts.filter(d => d.alert === 'RED' || d.alert === 'ORANGE').map(d => {
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="glass-panel p-5 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-400 font-mono">Source: IMD + AI Engine (Python)</span>
            </div>
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={activeHourly}>
                  <defs>
