...
const STATUS_CONFIG = {
  BLOCKED: {
    label: 'BLOCKED', icon: XCircle, color: 'text-red-400',
    bg: 'bg-red-500/10', border: 'border-red-500/40', badgeClass: 'badge-critical',
    dot: 'bg-red-500', barColor: '#ff3b5c',
  },
  CAUTION: {
    label: 'CAUTION', icon: AlertTriangle, color: 'text-amber-400',
    bg: 'bg-amber-500/10', border: 'border-amber-500/30', badgeClass: 'badge-warning',
    dot: 'bg-amber-500', barColor: '#ff9f1c',
  },
  CLEAR: {
    label: 'CLEAR', icon: CheckCircle2, color: 'text-emerald-400',
    bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', badgeClass: 'badge-safe',
    dot: 'bg-emerald-400', barColor: '#06d6a0',
  },
};
...
export default function RoadConnectivity() {
  ...
  // ── Fetched from Node.js API Gateway (/api/roads) ──────────────────────────
  const [roadData, setRoadData] = useState({ roads: [], dispatchLog: [], summary: {} });
  ...
  useEffect(() => {
    async function fetchRoads() {
      try {
        ...
        setRoadData(data);
        setSelectedRoad(data.roads[0] || null);
      } catch (err) {
  ...
  const ROAD_SEGMENTS = roadData.roads;
  const DISPATCH_LOG  = roadData.dispatchLog;
  const summary       = roadData.summary;
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-slate-400 text-sm gap-3"><RefreshCw className="animate-spin w-5 h-5 text-cyan-400" /> Fetching road data from API Gateway...</div>;
  if (error)   return <div className="p-6 text-center text-red-400 text-sm">⚠️ API Error: {error} — Is the Node.js backend running on port 5000?</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="glass-panel p-4 text-center space-y-1 border border-red-500/20">
          <div className="text-3xl font-extrabold text-red-400 font-mono">{summary.blocked ?? 0}</div>
        ...
        <div className="glass-panel p-4 text-center space-y-1 border border-amber-500/20">
          <div className="text-3xl font-extrabold text-amber-400 font-mono">{summary.caution ?? 0}</div>
        ...
        <div className="glass-panel p-4 text-center space-y-1 border border-emerald-500/20">
          <div className="text-3xl font-extrabold text-emerald-400 font-mono">{summary.clear ?? 0}</div>
        ...
        <div className="glass-panel p-4 text-center space-y-1 border border-cyan-500/20">
          <div className="text-3xl font-extrabold text-cyan-400 font-mono">{summary.totalVehicles ?? 0}</div>
